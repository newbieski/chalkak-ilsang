...
| ![캡션 확정](docs/screenshots/05_caption_confirmed.png) **⑤ 확정** — 생성된 문장을 직접 고친 뒤 확정하면 저장된다. | ![검색 결과](docs/screenshots/06_search_result.png) **⑥ 자연어 검색** — 태그·의미 기반 하이브리드 검색 결과를 사진과 함께 보여준다 (`contexts`·`trace`는 응답엔 담기지만 화면엔 안 띄우고 콘솔 로그로만 남김). |
...
검색 결과 화면은 `answer`(자연스러운 문장으로 정리, 언급된 사진 ID는 썸네일로도 같이 표시)만 보여준다. `contexts`/`trace`는 `POST /query` 응답에는 여전히 담겨 있지만(제출 규약상 세 키 고정), 화면에는 안 띄우고 브라우저 콘솔 로그로만 남긴다 — 에이전트가 무슨 근거로 어떻게 답했는지 확인하고 싶으면 개발자 도구 콘솔에서 볼 수 있다.
